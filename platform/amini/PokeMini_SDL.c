...
TUIMenu_Item UIItems_Platform[] = {
	PLATFORMDEF_GOBACK,
	{ 0,  1, "Display FPS: %s", UIItems_PlatformC },
...
void setup_screen()
{
	...
	PMWidth = 480; PMHeight = 272; PMOffX = 48; PMOffY = 8; UIOffX = 48; UIOffY = 8;
	...
	// Set video mode
	SDL_SetCursor(0);
	rl_screen = SDL_SetVideoMode(PMWidth, PMHeight, depth, SDL_HWSURFACE | SDL_ANYFORMAT | (clc_fullscreen ? SDL_FULLSCREEN : 0));
	screen = SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_ANYFORMAT, PMWidth, PMHeight, depth, 0,0,0,0);
	if (screen == NULL) {
