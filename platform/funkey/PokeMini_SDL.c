...
int UIItems_PlatformC(int index, int reason)
{
	...
	if (reason == UIMENU_LEFT) {
		switch (index)
		{
			case 2:
				CommandLine.scaling--;
				if (CommandLine.scaling < 0 || CommandLine.scaling > 2) CommandLine.scaling = 2;
				switch(CommandLine.scaling)
				{
					case 0:
						UIMenu_ChangeItem(UIItems_Platform, 2, "Scaling: %s", "Unscaled");
					break;
					case 1:
						UIMenu_ChangeItem(UIItems_Platform, 2, "Scaling: %s", "Fullscreen");
					break;
					case 2:
						UIMenu_ChangeItem(UIItems_Platform, 2, "Scaling: %s", "Keep aspect");
					break;
				}
			break;
		}
	...
	if (reason == UIMENU_RIGHT) {
		switch (index)
		{
			case 2:
				CommandLine.scaling++;
				if (CommandLine.scaling > 2) CommandLine.scaling = 0;
				switch(CommandLine.scaling)
				{
					case 0:
						UIMenu_ChangeItem(UIItems_Platform, 2, "Scaling: %s", "Unscaled");
...
void handleevents(SDL_Event *event)
{
	switch (event->type) {
	case SDL_KEYDOWN:
		if (event->key.keysym.sym == SDLK_ESCAPE || event->key.keysym.sym == SDLK_HOME) {		// Dingoo Select
...
int main(int argc, char **argv)
{
	...
	SDL_ShowCursor(0);
	const SDL_Surface* vidSurface = SDL_SetVideoMode(0, 0, 16, SDL_HWSURFACE | SDL_DOUBLEBUF);
	...
	rl_screen = SDL_SetVideoMode(width, height, 16, SDL_HWSURFACE | SDL_DOUBLEBUF);
	if (rl_screen == NULL) {
	...
	while (emurunning) {
		...
		if (SDL_LockSurface(screen) == 0) {
			...
			switch(CommandLine.scaling)
			{
				...
				break;
				case 2:
					bitmap_scale(16, 24, 192, 128, 240, 160, screen->w, 0, (uint16_t* restrict)screen->pixels, (uint16_t* restrict)rl_screen->pixels + 9600);
				break;
			}
			SDL_Flip(rl_screen);
		}
