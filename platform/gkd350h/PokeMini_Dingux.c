...
int main(int argc, char **argv)
{
	...
	PokeMini_InitDirs(home_path, NULL);
	CommandLineInit();
	...
	while (emurunning) {
		...
		if (SDL_LockSurface(screen) == 0) {
			...
			if (PokeMini_Rumbling) {
				SDL_FillRect(screen, NULL, 0);
				PokeMini_VideoBlit((uint16_t *)screen->pixels + 1 + PokeMini_GenRumbleOffset(PixPitch) + 16 + ( 24 * PixPitch ), PixPitch);
			} else {
				// 288 x 192
				// 320 x 240
				// offset: 16 x 24
				PokeMini_VideoBlit((uint16_t *)screen->pixels + 16 + ( 24 * PixPitch ), PixPitch);
			}
			LCDDirty = 0;
